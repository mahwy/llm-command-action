...
export class BamlAsyncClient {
  ...
  async GetReadPlan(
    ...
    try {
      ...
      return raw.parsed(false) as types.ReadPlannResult
    } catch (error) {
      throw toBamlError(error);
    }
  }
...
class BamlStreamClient {
  ...
  GetReadPlan(
      command: types.Command,
      __baml_options__?: { tb?: TypeBuilder, clientRegistry?: ClientRegistry, collector?: Collector | Collector[], env?: Record<string, string | undefined> }
  ): BamlStream<partial_types.ReadPlannResult, types.ReadPlannResult> {
    try {
      const options = { ...this.bamlOptions, ...(__baml_options__ || {}) }
      const collector = options.collector ? (Array.isArray(options.collector) ? options.collector : [options.collector]) : [];
      const rawEnv = __baml_options__?.env ? { ...process.env, ...__baml_options__.env } : { ...process.env };
      ...
      return new BamlStream<partial_types.ReadPlannResult, types.ReadPlannResult>(
        raw,
        (a): partial_types.ReadPlannResult => a,
        (a): types.ReadPlannResult => a,
        this.ctxManager.cloneContext(),
      )
    } catch (error) {
      throw toBamlError(error);
